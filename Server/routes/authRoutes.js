...
import { verifyToken, getUserByRole, checkLoginAttempts, resetLoginAttempts } from '../middleware/auth.js';
...
router.post("/register", async (req, res) => {
    try {
        const { name, email, phone, password, role, station, company } = req.body;
        if (!name || !email || !phone || !password || !role || !station) {
            return res.status(401).json({
                success: false,
                error: "Name,email,phone,password,role and station are required"
            });
        }

        const validateRole = ['admin', 'farmer', 'buyer', 'drymill', 'wetmill', 'finance'];
        if (!validateRole.includes(role)) {
            return res.status(400).json({
                success: false,
                error: `Invalid role. Allowed: ${validateRole.join(', ')}`
            });
        }
        ...
        switch (role) {
            ...
            case 'farmer':
                ...
                user = new Farmer({
                    id: farmerId, name, email, phone, password: hashedPassword, station: station || '', subscribed: false, verified: false
                });
                break;
            case 'buyer':
                user = new Buyer({
                    name, email, phone, password: hashedPassword, company: company || ''
                });
                break;
            case 'drymill':
            case 'wetmill':
            case 'finance':
                user = new Operator({
                    name, email, phone, password: hashedPassword, role: role, station: station || '', status: 'active'
                });
                break;
            default:
                return res.status(400).json({
        ...
        await user.save();
        try {
            let message = `Welcome ${name} to K-Trace Coffee grading system! Let's take from farm to cup`;
            if (role === 'farmer') {
...
router.post("/login", loginLimiter, async (req, res) => {
    try {
        ...
        if (!email || !password) {
            return res.status(400).json({
                sucess: false, error: "Email and password required"
            });
        }
        ...
        if (!userResult) {
            return res.status(404).json({
                success: false, error: "User not found. Proceed to registration"
            });
        }
        ...
        }, { expiresIn: process.env.JWT_EXPIRES_IN || '7d' }, process.env.JWT_SECRET);
...
router.put("/change-password", verifyToken, async (req, res) => {
    try {
        ...
        if (newPassword.length < 8) {
            res.status(400).json({ error: "New password must have at least 8 characters" });
        }
        ...
        if (!isMatch) {
            return res.status(401).json({
                success: false, error: "Initial password entered is incorrect"
            });
        }
        res.json({
            success: true,
