...
router.post("/ussd", (req, res) => {
    const ussd = africastalkingC.USSD;
...
7. Exit`;
        ...
        response = `END Today's coffee price is KSh ${latestPrice?.amount || 85} per kg`;
    } else if (text[0] === "2") {
        await Farmer.findOneAndUpdate({ phone: phoneNumber }, { subscribed: true }, { upsert: true });
        response = `END You have subscribed to daily coffee price alerts`;
    } else if (text[0] === "3") {
        await Farmer.findOneAndUpdate({ phone: phoneNumber }, { subscribed: false });
        ...
        if (text.length === 1) {
            ...
            await Farmer.findOneAndUpdate(
                { phone: phoneNumber },
                { id, name, station, phone: phoneNumber, otp, verified: false },
                { upsert: true }
            );
            ...
            if (farmer && farmer.otp === enteredOtp) {
                ...
                farmer.otp = null;
                await farmer.save();
                await sendSMS(phoneNumber, `Welcome ${farmer.name} to Agri Farmers Co-Op! Your ID: ${farmer.id}`);
                response = `END Registration complete! Welcome ${farmer.name} to Agri farmers Co-op 🌱`;
            } else {
        ...
        if (!farmer) {
            ...
            if (text.length === 1) {
                response = `CON What would you like to update?
1. Name
2. Station`;
            } else if (text.length === 2) {
                if (text[1] === "1") {
